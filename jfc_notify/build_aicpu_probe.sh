...
if [[ ! -r "$protocol_dir/secure_dma_urma_probe_protocol.h" ]]; then
    printf 'Copy secure_dma_urma_probe_protocol.h beside this script, or set AICPU_ROOT / PROBE_PROTOCOL_DIR.\n' >&2
    exit 1
fi
...
"$probe_cxx" -std=c++14 -O2 -Wall -Wextra -Werror \
    ...
    -I"$protocol_dir" "$probe_dir/aicpu_urma_probe.cpp" \
...
printf 'After deploying the updated CI AICPU package: run %s --device 0\n' "$probe_output"
